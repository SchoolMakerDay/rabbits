...
<ul>
    ...
    <li>coppia chiave2-valore read-only in questa interfaccia (esprime uno stato ricevuto dal dispositivo IOT</li>
</ul>
<div id="statuskeydiv">
            ...
            <form id="statuskeyform" method="GET">
                ...
                StatusKey2: <input id='statuskey2' name='statuskey2'><br>
                <input type="submit">
            </form>
</div>      
<div id="command">      
...
<div id="monitor">            
    ...
    <h2>Stato per key=<span id='statuskey2Value'></span></h2>
    <div id='statusstatus2'></div>
    <div id='statusdata2'>
    <p>Value=<span id='statusvalue2Value'></span></p>
    <p>Ts=<span id='statusts2Value'></span></p>
    </div>
	<hr>
...
//    $.ajax("<?=$getvalueUrl?>"+statusKey).done(refreshCallback);
    $.ajax("<?=$getvalueUrl?>"+statusKey2).done(refreshCallback2);
}
...
function refreshCallback2(data){
    if (data.status=='OK'){
        if (data.data){
            $("#statusdata2").show();
            $("#statusstatus2").hide();
            $("#statusvalue2Value").html(data.data.value);
            $("#statusts2Value").html(data.data.ts);
        }
        else {
            $("#statusdata2").hide();
            $("#statusstatus2").show();
            $("#statusstatus2").html('<p>Key not found</p>');
        }
    }
    else {
        $("#statusdata2").hide();
        $("#statusstatus2").show();
        $("#statusstatus2").html('<p>Error in API call</p>');
    }    
    $("#updating").hide();
    d=new Date();
    ora=d.toTimeString().replace(/.*(\d{2}:\d{2}:\d{2}).*/, "$1");
